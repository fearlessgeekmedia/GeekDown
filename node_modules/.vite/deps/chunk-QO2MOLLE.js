...
var c = (component, options2 = HTMLElement) => {
  ...
  class AtomicoElement extends base {
    ...
    attributeChangedCallback(attr, oldValue, value) {
      if (attrs[attr]) {
        if (attr === this._ignoreAttr || oldValue === value)
          return;
...
var useCallback = (callback, args) => useMemo(() => callback, args);
...
function setProperty(node, key, prevValue, nextValue, isSvg, handlers) {
  ...
  if (nextValue === prevValue || INTERNAL_PROPS[key] || key[0] == "_")
    return;
  if (node.localName === "slot" && key === "assignNode" && "assign" in node) {
    ...
    if (nextIsObject) {
      for (const key2 in nextValue) {
        ...
        if (prevIsObject && prevValue[key2] === value)
          continue;
...
// ../node_modules/@milkdown/crepe/lib/esm/functions-Bsik6ikd.js
var alignCenterIcon = html`
  <svg
...
`;
var checkBoxUncheckedIcon = html`
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
...
//# sourceMappingURL=chunk-QO2MOLLE.js.map
